...
```
The above implementation uses plain JS objects, but would look about the same using a Map() or WeakMap(). An easy extra optimization that can be made with this approach is defining the object externally to the function in order to limit how many of these objects are created in memory. Then the function is simply a getter for that object, essentially.

```js
const Animals = {cat: 'meow', ... } // same obj as above just defined outside function.
function lookupSound(str) {
  return Animals[str] ?? 'silence'
}
```
Going this route suggests that the lookupSound function could be abstracted to a general lookupValue function and could be used on any object that you define in your application:
```js
function lookupValue(obj, str, default = 'silence') {
